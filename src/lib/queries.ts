...
function get_params(
  ...
  const common_keys = ['include_audible', 'categories', 'filter_categories', 'bid_browsers', 'bid_stopwatch', 'return_variable_suffix'] as const;
  
  // Create base params
  const base_params: DesktopQueryParams = {
    bid_window: 'aw-watcher-window_' + host,
    bid_afk: 'aw-watcher-afk_' + host,
    bid_browsers: [],
    return_variable_suffix: '',
    filter_afk: params.filter_afk,
    always_active_pattern: params.always_active_pattern,
    categories: params.categories || [],
    filter_categories: params.filter_categories || []
  };

  // Copy common parameters if they exist
  common_keys.forEach(key => {
    if (params[key as keyof MultiQueryParams] !== undefined) {
      (base_params as any)[key] = params[key as keyof MultiQueryParams];
    }
  });

  // Copy host-specific parameters
  Object.entries(host_params).forEach(([key, value]) => {
    if (value !== undefined) {
      (base_params as any)[key] = value;
    }
  });

  if (!isDesktopParams(base_params)) {
    console.error(`Invalid params for host ${host}: ${JSON.stringify(base_params)}`);
  }
...
function browsersWithBuckets(browserbuckets: string[] | undefined): [string, string][] {
  if (!browserbuckets) return [];
  
  const browsername_to_bucketid = Object.entries(browser_appnames).map(([browserName]) => {
    const bucketId = browserbuckets.find(bid => bid.includes(browserName.toLowerCase()));
    return [browserName, bucketId] as [string, string | undefined];
  });

  return browsername_to_bucketid.filter((pair): pair is [string, string] => pair[1] !== undefined);
}
...
function browserEvents(params: DesktopQueryParams): string {
  ...
  const browserBuckets = params.bid_browsers || [];
  browsersWithBuckets(browserBuckets).forEach(([browserName, bucketId]) => {
    // Type assertion since we know browserName is a valid key
    const browserAppNames = browser_appnames[browserName as keyof typeof browser_appnames];
    const browser_appnames_str = JSON.stringify(browserAppNames);
